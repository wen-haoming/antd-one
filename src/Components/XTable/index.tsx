...
interface Props {
  toolbarRender?: (
    rowKeys: string[],
    rows: any[],
  ) => ReactNode | ((refresh: () => void, rowKeys: string[], rows: any[]) => ReactNode);
  refresh?: () => void;
}

export const XTable: FC<Props & TableProps<any> & any> = (props) => {
  const [selectedRowKeys, setSelectedRowKeys] = useState([]);
  const [selectedRows, setSelectedRows] = useState([]);
  const { toolbarRender, scroll, rowSelection, pagination, refresh, ...rest } = props;
