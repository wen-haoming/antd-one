...
import type { TableFormRenderProps } from './types';
...
export const TableFormRender = (props: TableFormRenderProps) => {
  const {
    request,
  ...
  if (tableOptions?.toolbarRender && tableOptions.isCheckbox && !tableOptions.rowSelection) {
  ...
  const fields: FRField = columns.filter((obj) => obj.searchField).map((item) => item.searchField);
  ...
  return (
    <>
      <Card>
        ...
        >
          <Render
            formDataOptions={formInstance.formDataOpts}
            form={formInstance}
            install={install}
            fields={fields}
          />
...
export default memo(TableFormRender);
