...
function TableFormRender<T extends Record<string, JSXComponent>>(
  props: TableFormRenderProps<T>,
) {
  const {
    request,
    requestOptions,
  ...
  const { gridProps = { maxColumns: 4, minColumns: 4 }, layoutProps } =
    formProps || {};
  ...
  const fiels = useMemo(() => {
    return toJS(
      columns
        .filter((item) => item && !!item.searchField)
        .map((field) => {
...
TableFormRender.connect = connect;
TableFormRender.mapProps = mapProps;
TableFormRender.mapReadPretty = mapReadPretty;
